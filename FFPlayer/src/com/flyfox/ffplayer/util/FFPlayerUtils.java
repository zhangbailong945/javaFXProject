...
import com.flyfox.ffplayer.util.mp3.MP3Info;
import com.flyfox.ffplayer.util.mp3.MP3Utils;
...
public class FFPlayerUtils {
	...
	public static FFPlayer getFFPlayer(String path) {
		...
		try {
			...
			if (mp3Info != null) {
				if (!FFPlayerUtils.isMessyCode(mp3Info.getTitle())) {
					model.setTitle(mp3Info.getTitle());
				}
				if (!FFPlayerUtils.isMessyCode(mp3Info.getAlbum())) {
					model.setAlbum(mp3Info.getAlbum());
				}
				if (!FFPlayerUtils.isMessyCode(mp3Info.getArtist())) {
