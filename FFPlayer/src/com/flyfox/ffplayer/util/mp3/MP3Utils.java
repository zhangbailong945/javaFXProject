package com.flyfox.ffplayer.util.mp3;
...
public class MP3Utils {
	...
	private static MP3Info readV1(RandomAccessFile raf) throws IOException {
		...
		if (StrUtils.isNotEmpty(songName) && !FFPlayerUtils.isMessyCode(songName)) {
			mp3Info.setTitle(songName);
		}
		String artist = new String(buf, 30, 30, "GBK").trim();// 歌手名字
		if (StrUtils.isNotEmpty(artist) && !FFPlayerUtils.isMessyCode(artist)) {
			mp3Info.setArtist(artist);
		}
		// 专辑
		String album = new String(buf, 60, 30, "GBK").trim();// 专辑名称
		if (StrUtils.isNotEmpty(album) && !FFPlayerUtils.isMessyCode(album)) {
